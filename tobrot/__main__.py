...
if __name__ == "__main__" :
    ...
    incoming_message_handler = MessageHandler(
        incoming_message_f,
        filters=Filters.command(["leech@Leech_jinn_robot"]) & Filters.chat(chats=AUTH_CHANNEL)
    )
    ...
    incoming_youtube_dl_handler = MessageHandler(
        ...
        filters=Filters.command(["ytdl@Leech_jinn_robot"]) & Filters.chat(chats=AUTH_CHANNEL)
    )
    ...
    upload_document_handler = MessageHandler(
        upload_document_f,
        filters=Filters.command(["upload@Leech_jinn_robot"]) & Filters.chat(chats=AUTH_CHANNEL)
    )
    app.add_handler(upload_document_handler)

    help_text_handler = MessageHandler(
        help_message_f,
        filters=Filters.command(["help@Leech_jinn_robot"]) & Filters.chat(chats=AUTH_CHANNEL)
    )
    ...
    save_thumb_nail_handler = MessageHandler(
        save_thumb_nail,
        filters=Filters.command(["savethumbnail@Leech_jinn_robot"]) & Filters.chat(chats=AUTH_CHANNEL)
    )
    app.add_handler(save_thumb_nail_handler)
    #
    clear_thumb_nail_handler = MessageHandler(
        clear_thumb_nail,
        filters=Filters.command(["clearthumbnail@Leech_jinn_robot"]) & Filters.chat(chats=AUTH_CHANNEL)
    )
